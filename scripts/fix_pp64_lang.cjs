// Generic PP64 fix for any language directory
// Usage: node scripts/fix_pp64_lang.cjs <lang-dir>
const fs = require('fs');
const path = require('path');

const langDir = process.argv[2];
if (!langDir) {
  console.error('Usage: node fix_pp64_lang.cjs <lang-v2>');
  process.exit(1);
}
...
function appendMatchBlock(src, pairs) {
  ...
  // Chunk pairs to prevent >8 items per match (readability)
  const chunks = [];
  ...
  return src.substring(0, lastLessonClose) + matchBlocks + src.substring(lastLessonClose);
}
