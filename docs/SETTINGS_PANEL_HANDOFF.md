...
5 languages x ~250 lessons each = ~1,250 lessons to reformat. Even with content salvage, every lesson needs: CEFR-scaled dialogue cards with fun info + gender coloring + grammar tags, story arc per unit, interleaved flow. Estimate: 5-10 sessions PER LANGUAGE for full restructure. That's 25-50 sessions for all 5 languages. The prototype (steps 5-6, ONE language) will reveal the real pace.
