...
{id:"jav2_u11l1", title:"て-形(けい)をつくろう", icon:"\u{1F6E0}\uFE0F", xp:15, board:true, steps:[
  ...
  {type:"teach", trg:"話(はな)して", src:"speak (て-form)", pos:"verb", gender:null,
   ...
   funFact:"The す to して change is the most regular pattern in Group 1. Every す-ending verb follows it without exception. Common examples: 出(だ)す (to put out) becomes 出(だ)して, 消(け)す (to erase) becomes 消(け)して."},
  ...
  {type:"match", pairs:[{trg:"食(た)べて",src:"eat (て)"},{trg:"飲(の)んで",src:"drink (て)"},{trg:"書(か)いて",src:"write (て)"},{trg:"待(ま)って",src:"wait (て)"}]},

  {type:"match", pairs:[{trg:"話(はな)して",src:"speak (て)"},{trg:"して",src:"do (て)"},{trg:"来(き)て",src:"come (て)"},{trg:"食(た)べて",src:"eat (て)"}]},

  {type:"fb", s:"名前(なまえ)を{1}ください。\n(Please write your name.)", a:"書(か)いて", opts:["書(か)いて","書(か)って","書(か)きて","書(か)くて"], sSrc:"Please write your name.",
...
{id:"jav2_u11l2", title:"て-形(けい)の使(つか)い方(かた)", icon:"\u2728", xp:15, board:true, steps:[
  ...
  {type:"teach", trg:"座(すわ)って", src:"sit (て-form)", pos:"verb", gender:null,
   ...
   example:"A: ここに座(すわ)ってください。\nB: ありがとうございます。",
   exampleSrc:"A: Please sit here.\nB: Thank you.",
   funFact:"座(すわ)る is a Group 1 verb even though it ends in る. The key test: if the vowel before る is not -i or -e, it is Group 1. 座(すわ)る has -a before る, so it is Group 1. This is the trickiest part of verb classification for learners."},

  {type:"fb", s:"今(いま)本(ほん)を{1}います。\n(I am reading a book now.)", a:"読(よ)んで", opts:["読(よ)んで","読(よ)みて","読(よ)って","読(よ)むて"], sSrc:"I am reading a book now.",
  ...
  {type:"teach", trg:"立(た)っています", src:"is standing", pos:"verb", gender:null,
   note:"立(た)つ (to stand) in the ongoing form: 立(た)って + います.\nDescribes the state of standing.",
   example:"A: あの人(ひと)は誰(だれ)ですか?\nB: 駅(えき)の前(まえ)に立(た)っていますか? あれは山田(やまだ)さんです。",
  ...
  {type:"match", pairs:[{trg:"食(た)べてください",src:"please eat"},{trg:"食(た)べています",src:"is eating"},{trg:"食(た)べてもいいですか",src:"may I eat?"},{trg:"飲(の)んでください",src:"please drink"}]},

  {type:"fb", s:"{1}を撮(と)ってもいいですか?\n(May I take a photo?)", a:"写真(しゃしん)", opts:["写真(しゃしん)","映画(えいが)","テレビ","音楽(おんがく)"], sSrc:"May I take a photo?",
