...
**Validator state (2026-04-24 — extended session):** Extended cleanup continued through PR #563. Additional milestones: (1) Japanese template notes ZEROED — 192 "JLPT N2/N1 level vocabulary" template notes across batch9+batch10 u25-u36 replaced with specific hanja breakdowns. (2) Mojibake cleanup — 30+ UTF-8 replacement characters (��) fixed across Japanese/Korean/Spanish unit files and Dutch foundations. (3) POS corrections — 400+ miscategorized teach cards corrected (Dutch, Spanish, French, Italian, Portuguese, Russian across numerals, past participles, conditional/subjunctive verbs, adverbs, pronouns, articles, colors, adjectives, months, proper noun genders, phrase patterns). (4) **Gender zeroed for all 7 gendered languages** — German/Dutch/French/Spanish/Italian/Portuguese/Russian all now have 0 noun cards missing gender (was 488 missing total at session start; down to 0). Runtime validator: 0 violations sustained throughout.

**Validator state (2026-04-26 — foundation overhaul, 30+ autonomous PRs):** PRs #632–#664. Added Rules H12 (queue-first, no handoff drift) + I (validator-first engineering) to CLAUDE.md. Codified 5 NEW BLOCKING validators in CI strict mode: `audit_pos_gender.mjs` (passes 13-21), `audit_hint_quality.mjs` (passes 1-11), `audit_a11y.mjs` (WCAG AA proxy, baseline 0), `audit_bundle_size.mjs` (locks 2026-04-26 chunk sizes + 15% slack), `audit_visual_consistency.mjs` (informational, surfaces 1,061 debt points for Claude Design). Pre-commit gates upgraded to match. **A11y: 76 → 0 violations** across imgs/buttons/icon-buttons/clickable-divs/inputs, locked in CI strict gate. **Bundle: 47 MB index chunk locked at +15% slack budget.** All other validators (PP8/22c/43/48/63/64/66/67) sustained at 0. Major shipped features: Expert Mode (B1+ free-text), PWA Phase 1+2 + install prompt + update banner + Google Fonts caching, AI conversation Phase 1 (unit-scoped vocab tutor + Clear button + role-play prompts), brand purple ramp tokens, :focus-visible defaults, skip-to-content link, role="main" landmark, per-page document.title, html lang sync with source language, prefers-reduced-motion support (WCAG 2.3.3), autocomplete attrs (WCAG 1.3.5), input lang attrs (WCAG 3.1.2), OS prefers-color-scheme dark mode default, noscript fallback, OpenGraph/Twitter/Apple PWA meta tags. See merged PR list #632-#664.
